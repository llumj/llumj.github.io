...
🚨 Degf (degree Fahrenheit): For each 1°F increase, the LD cases are expected to increase by approximately 18.158 cases. p-value (5.25e-09) indicates this relationship is statistically significant.🚨
...
🚨 Inches (6.008): For each additional inch, the number of LD cases is expected to increase by approximately 6.008. This relationship is also statistically significant given the very low p-value (0.000466).🚨
...
The model above revealed several key insights into the factors influencing Lyme disease cases in Maine. P-values for the temperature (degf) and rainfall (inches) are significant predictors of Lyme disease cases, with both variables showing a positive relationship with the number of cases. This means that as temperatures and precipitation increase, so do the instances of Lyme disease. Additionally, coastal was not significant in predicting Lyme disease cases. This means ***we fail to reject the null hypothesis***. Overall, the model explains approximately 38.27% of the variance in Lyme disease cases, indicating that while these factors are important, other variables may also play a role in driving the trends we observe. 
...
Change: The coefficient for degf changed from positive to negative and became non-significant. This suggests that once population is accounted for, the impact of temperature on Lyme Disease cases is not as substantial.
